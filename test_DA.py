...
d = Dataloader('data/PTB_XL_data/signals_train.hdf5', 'data/PTB_XL_data/train_data.csv',None,None, DA_P=1)
base_dataset, label = d.getTrainingData_Plot(sliceIdx=2)
(b_sample) = base_dataset.take(2)
...
def Host_Guest_ECG_Augmentation_12_Leads(data,file_path,Lead_No,Level,threshold):  
    ...
    for ind in df.index:
        ...
        if len(original_df)== 2 and max(original_df) == ind:
            ...
            augmented_data = Modified_measure_tolerance(df.index.tolist(),data,file_path,current,ind,Lead_No,Max_Original_df_index,
...
def main():
    ...
    file = "y"
    No_of_leads=12
    Level=4 #defines how many sample needs to be create from one original sample i.e., when 3 is given it will create 3 new from each original signal (Level must be greater than 2)
    ...
    for Lead in range (0, No_of_leads,1): #initiating 12 times loop on variable x to extract lead/column wise data (which then passed to Host_Guest_ECG_Augmentation_12_Leads method)
        ...
        data_set = Host_Guest_ECG_Augmentation_12_Leads(n,file,Lead,Level,threshold)
    return data_set
...
# plt.plot(b.numpy()[:,3], label="Base ", alpha=0.7)
# plt.plot(data_set[1][:,3], label="Aug ", alpha=0.7)
# plt.legend()
# plt.grid()
# plt.show()
